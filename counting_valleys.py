...
def countingValleys(steps, path):
    ...
    for step in path:
        if step == "U":
            count += 1
            altitude.append(count) 
        else:
            count -= 1
            altitude.append(count)
        
    for x in range(1,len(altitude)):
        if (altitude[x] == -1 and altitude[x-1] == 0) or (altitude[x] == 0 and altitude[x-1] == -1):
            valleys += 1

    return math.ceil(valleys/2)


print(countingValleys(8,"UDDDUDUU")) ##Expected Output = 1
print(countingValleys(12,"DDUUDDUDUUUD")) ##Expected Output = 2
print(countingValleys(10,"UDUUUDUDDD")) ##Expected Output = 0